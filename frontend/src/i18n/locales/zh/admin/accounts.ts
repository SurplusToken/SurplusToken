export default {
    accounts: {
      ...
      owners: {
        ...
        primaryOwnerPlaceholder: '用户 ID，留空表示清除',
        primaryOwnerHint: '设置主要所有者后，管理员添加的 api-key 等账号即成为“贡献账号”，可加入动态分组并使用动态倍率。留空清除。',
        userIdLabel: '用户 ID {id}',
        noPrimaryOwner: '无主要所有者',
        sharingRate: '动态共享倍率',
        sharingRatePlaceholder: '例如：1.5',
        sharingRateHint: '动态分组中的市场计费倍率，仅对已设主要所有者的可调度账号生效。留空则不修改。',
        sharingRateNeedsOwner: '需先设置主要所有者才能设置共享倍率',
        coOwners: '协同所有者用户 ID',
      ...
      openai: {
        ...
        testStoreFalse: '测试请求强制带 store:false',
        testStoreFalseDesc: 'OpenAI Responses 账号测试时携带 store:false。中转上游（如 CC Host）要求该字段时开启；OAuth 账号始终携带，无需开启。',
