      ...
      <template v-if="notifyEnabled">
        ...
        <div>
          <label class="input-label">
            ...
            <span class="ml-2 text-xs text-muted-foreground">{{ t('profile.balanceNotify.thresholdHint') }}</span>
          </label>
        ...
        <div>
          ...
          <div v-if="emailEntries.length > 0" class="space-y-2 mb-3">
            <div v-for="(entry, idx) in emailEntries" :key="idx"
              class="flex items-center justify-between px-3 py-2 rounded-lg bg-muted">
              <div class="flex items-center gap-2 min-w-0 flex-1">
                ...
                <span class="truncate text-sm text-foreground">{{ entry.email }}</span>
              </div>
