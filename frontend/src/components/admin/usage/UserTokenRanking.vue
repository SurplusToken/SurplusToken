<template>
  ...
  <div>
    ...
    <div class="flex flex-wrap items-center justify-between gap-3 border-b border-border px-4 py-3 sm:px-6">
      <p class="text-xs text-muted-foreground">{{ t('admin.usage.tokenRanking.subtitle') }}</p>
      <div class="flex items-center gap-3">
        <span v-if="!loading && items.length > 0" class="text-xs text-muted-foreground">
    ...
    <div class="overflow-x-auto">
      <table class="w-full min-w-max divide-y divide-border">
        <thead class="bg-muted">
          <tr>
            <th class="w-16 px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-muted-foreground sm:px-6">#</th>
            ...
            <th
              ...
              :key="col.key"
              class="cursor-pointer select-none whitespace-nowrap px-4 py-3 text-right text-xs font-medium uppercase tracking-wider transition-colors hover:bg-muted"
              :class="sortBy === col.key ? 'text-primary-600 dark:text-primary-400' : 'text-muted-foreground'"
        ...
        <tbody class="divide-y divide-border bg-card">
          ...
          <tr v-else-if="items.length === 0">
            <td :colspan="sortableColumns.length + 2" class="py-12 text-center text-sm text-muted-foreground">
          ...
          >
            <td class="px-4 py-3 sm:px-6">
              ...
              >{{ index + 1 }}</span>
              <span v-else class="inline-block w-6 text-center text-sm tabular-nums text-muted-foreground">{{ index + 1 }}</span>
            </td>
            <td class="max-w-[260px] truncate px-4 py-3 text-sm font-medium text-foreground" :title="item.email">
              {{ item.email || `User #${item.user_id}` }}
              <span class="ml-1 font-normal text-muted-foreground">#{{ item.user_id }}</span>
            </td>
            <td class="whitespace-nowrap px-4 py-3 text-right text-sm tabular-nums text-muted-foreground">{{ item.requests.toLocaleString() }}</td>
            <td class="whitespace-nowrap px-4 py-3 text-right text-sm tabular-nums text-muted-foreground">{{ fmtTokens(item.input_tokens) }}</td>
            <td class="whitespace-nowrap px-4 py-3 text-right text-sm tabular-nums text-muted-foreground">{{ fmtTokens(item.output_tokens) }}</td>
            <td class="whitespace-nowrap px-4 py-3 text-right text-sm tabular-nums text-muted-foreground">{{ fmtTokens(item.cache_tokens) }}</td>
            ...
            <td class="whitespace-nowrap px-4 py-3 text-right text-sm font-medium tabular-nums text-foreground">${{ fmtCost(item.actual_cost) }}</td>
          </tr>
        </tbody>
      </table>
    </div>
