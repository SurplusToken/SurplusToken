<template>
  <BaseDialog :show="show" :title="t('admin.usage.cleanup.title')" width="wide" @close="handleClose">
    <div class="space-y-4">
      ...
      <div class="rounded-lg border border-amber-600/20 bg-amber-50/50 px-4 py-3 text-sm text-amber-700 dark:bg-amber-500/10 dark:text-amber-300">
        ...
        <div class="mt-3 space-y-2">
          ...
          <div v-else class="space-y-2">
            <div
              v-for="task in tasks"
              :key="task.id"
              class="flex flex-col gap-2 rounded-lg border border-border px-3 py-2 text-sm text-muted-foreground"
            >
              <div class="flex flex-wrap items-center justify-between gap-2">
                <div class="flex items-center gap-2">
                  <span :class="statusClass(task.status)" class="rounded px-2 py-0.5 text-xs font-medium ring-1 ring-inset">
...
const statusClass = (status: string) => {
  const map: Record<string, string> = {
    pending: 'bg-amber-50 text-amber-700 ring-amber-600/20 dark:bg-amber-900 dark:text-amber-300',
    running: 'bg-blue-50 text-blue-700 ring-blue-600/20 dark:bg-blue-900 dark:text-blue-300',
    succeeded: 'bg-emerald-50 text-emerald-700 ring-emerald-600/20 dark:bg-emerald-900 dark:text-emerald-300',
    failed: 'bg-rose-50 text-rose-700 ring-rose-600/20 dark:bg-rose-900 dark:text-rose-300',
    canceled: 'bg-gray-50 text-gray-700 ring-gray-600/20 dark:bg-gray-900 dark:text-gray-300'
  }
  return map[status] || 'bg-gray-50 text-gray-700 ring-gray-600/20 dark:bg-gray-900 dark:text-gray-300'
}
