          ...
          <div v-if="paidOrder" class="w-full rounded-lg bg-muted p-4">
            <div class="space-y-2 text-sm">
              ...
              <div class="flex justify-between">
                ...
                <span class="font-medium text-foreground">{{ creditedAmountSymbol }}{{ paidOrder.amount.toFixed(2) }}</span>
              </div>
              <div class="flex justify-between">
                <span class="text-muted-foreground">{{ t('payment.orders.payAmount') }}</span>
                <span class="font-medium text-foreground">{{ formatGatewayAmount(paidOrder.pay_amount, paidOrder.currency) }}</span>
              </div>
            </div>
          </div>
      ...
      <template v-if="!deepLinkFallbackVisible">
        <div class="rounded-lg border border-border bg-card p-6">
          <div class="flex flex-col items-center space-y-4 py-4 text-center">
            ...
            <p class="text-sm text-muted-foreground">{{ t('payment.qr.alipayWaitingHint') }}</p>
            <button
        ...
        <div class="rounded-lg border border-border bg-card p-4 text-center">
          ...
          <p class="mt-1 text-xl font-semibold tabular-nums text-foreground">{{ countdownDisplay }}</p>
          <p class="mt-1 text-xs text-muted-foreground">{{ t('payment.qr.waitingPayment') }}</p>
        </div>
      </template>
      <template v-else>
        <div data-test="alipay-qr-fallback" class="rounded-lg border border-border bg-card p-6">
          <div class="flex flex-col items-center space-y-4">
            <div class="text-center">
              <p class="text-lg font-semibold text-foreground">{{ t('payment.qr.alipayFallbackTitle') }}</p>
              <p class="mt-1 text-sm text-muted-foreground">{{ t('payment.qr.alipayFallbackHint') }}</p>
            </div>
            <div class="w-full space-y-2 border-y border-border py-3 text-sm">
              ...
              <div class="flex items-start justify-between gap-4">
                <span class="text-muted-foreground">{{ t('payment.qr.expiresIn') }}</span>
