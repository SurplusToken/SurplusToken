...
const readCachedResetCredits = (account: QuotaAccountLike): OpenAIQuotaUsage | null => {
...
const handleQuery = async () => {
  ...
  try {
    if (props.scope === 'user') {
      data.value = await queryOwnerOpenAIQuota(props.account.id)
    } else {
      const result = await refreshOpenAIQuota(props.account.id)
      // The upstream read succeeded even when the snapshot write was rejected,
      // so the live count is always adopted. Only the persisted view is left
      // alone, keeping displayed expirations consistent with stored state.
      data.value = result
...
const confirmReset = async () => {
  ...
  try {
    if (props.scope === 'user') {
      const result: OpenAIQuotaResetResult = await resetOwnerOpenAIQuota(props.account.id)
      // The owner endpoint retains its read-only query contract, so refresh the
      // count explicitly after consuming a credit.
      await handleQuery()
