...
<template>
  <div v-if="allEndpoints.length > 0" class="flex flex-wrap gap-2">
    <div
      ...
      :key="index"
      class="flex items-center gap-1.5 rounded-lg border border-border bg-card px-2.5 py-1.5 text-xs transition-colors hover:border-primary-300 dark:hover:border-primary-700"
    >
      ...
      <div class="group/endpoint relative flex items-center gap-1.5">
        <div
          class="pointer-events-none absolute bottom-full left-1/2 z-20 mb-2 w-max max-w-[24rem] -translate-x-1/2 translate-y-1 rounded-lg border border-border bg-card px-3 py-2.5 text-left opacity-0 shadow-lg transition-all duration-150 group-hover/endpoint:translate-y-0 group-hover/endpoint:opacity-100 group-focus-within/endpoint:translate-y-0 group-focus-within/endpoint:opacity-100"
        >
          <p
            v-if="item.description"
            class="max-w-[24rem] break-words text-xs leading-5 text-muted-foreground"
          >
            {{ item.description }}
          </p>
          <p
        ...
        <code
          class="cursor-pointer font-mono text-muted-foreground decoration-gray-400 decoration-dashed underline-offset-2 hover:text-primary-600 hover:underline focus:text-primary-600 focus:underline focus:outline-none dark:decoration-gray-500 dark:hover:text-primary-400 dark:focus:text-primary-400"
          role="button"
          tabindex="0"
        ...
        <button
          ...
          :class="copiedEndpoint === item.endpoint
            ...
            : 'text-muted-foreground hover:text-primary-500 dark:hover:text-primary-400'"
        ...
        <a
          ...
          class="rounded p-0.5 text-muted-foreground transition-colors hover:text-primary-500 dark:hover:text-primary-400"
