    ...
    <div class="space-y-6">
      ...
      <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center">
          <!-- Scope selector: 全平台 / a subscription group (订阅) -->
          <select
            v-if="groups.length"
            v-model="scope"
      ...
      <!-- Per-user token usage trend (platform scope only; one line per user) -->
      <section
        v-if="scope === 'platform'"
      ...
      <template v-else>
        ...
        <template v-else>
          ...
          <!-- "Your rank" card (platform scope only) -->
          <div
            v-if="scope === 'platform'"
            class="rounded-lg border border-gray-200 bg-white p-4 shadow-sm dark:border-dark-700 dark:bg-dark-800"
          >
...
// ---- Scope: 'platform' (whole platform) or a group id (订阅) ----
type Scope = 'platform' | number
const scope = ref<Scope>('platform')
const groups = ref<Group[]>([])

// 'month' shows as "本月" platform-wide, "订阅月" within a subscription group.
function periodLabel(p: LeaderboardPeriod): string {
...
function isMe(entry: LeaderboardEntry): boolean {
  return scope.value === 'platform' && currentUserId.value != null && entry.user_id === currentUserId.value
}
...
async function load() {
  ...
  error.value = false
  try {
    const groupId = scope.value === 'platform' ? undefined : (scope.value as number)
...
async function loadTrend() {
  if (scope.value !== 'platform') return
