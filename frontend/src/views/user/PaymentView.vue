  ...
  <AppLayout>
    <div class="mx-auto max-w-4xl space-y-6">
      ...
      <template v-else>
        ...
        <div v-if="tabs.length > 1 && paymentPhase === 'select' && !selectedPlan" class="flex space-x-1 rounded-lg bg-muted p-0.5">
          <button v-for="tab in tabs" :key="tab.key"
            class="flex-1 rounded-md px-4 py-2 text-sm font-medium transition-colors"
            :class="activeTab === tab.key ? 'bg-white text-foreground shadow-sm dark:bg-dark-700' : 'text-muted-foreground hover:text-foreground'"
        ...
        <template v-else>
          ...
          <template v-if="activeTab === 'recharge'">
            ...
            <div class="rounded-lg border border-border bg-card p-5">
              ...
              <p class="mt-0.5 text-sm font-medium text-muted-foreground">{{ t('payment.currentBalance') }}: {{ user?.balance?.toFixed(2) || '0.00' }}</p>
            </div>
            <div v-if="enabledMethods.length === 0" class="rounded-lg border border-border bg-card py-16 text-center">
            ...
            <div v-if="enabledMethods.length >= 1" class="rounded-lg border border-border bg-card p-6">
            ...
            <div v-if="validAmount > 0" class="rounded-lg border border-border bg-card p-6">
              <div class="space-y-2 text-sm">
                ...
                <div v-if="feeRate > 0" class="flex justify-between border-t border-border pt-2">
                  ...
                  <span class="text-xl font-semibold text-foreground">{{ formatSelectedPaymentAmount(totalAmount) }}</span>
                </div>
          ...
          <template v-else-if="activeTab === 'subscription'">
            ...
            <template v-if="selectedPlan">
              <div class="rounded-lg border border-border bg-card p-5">
                ...
                <div class="mb-3 flex flex-wrap items-center gap-2">
                  ...
                  <h3 class="text-lg font-semibold text-foreground">{{ selectedPlan.name }}</h3>
                </div>
                <!-- Price -->
                <div class="flex items-baseline gap-2">
                  ...
                  <span class="text-xl font-semibold text-foreground">{{ formatSelectedSubscriptionPaymentAmount(selectedPlan.price) }}</span>
                ...
                <div class="mt-3 grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <div class="flex items-baseline">
                      <span class="text-xl font-semibold text-foreground">×{{ selectedPlan.rate_multiplier ?? 1 }}</span>
                    </div>
                  </div>
                  ...
                  <div v-if="selectedPlan.daily_limit_usd != null">
                    ...
                    <div class="text-xl font-semibold text-foreground">${{ selectedPlan.daily_limit_usd }}</div>
                  </div>
                  <div v-if="selectedPlan.weekly_limit_usd != null">
                    <span class="text-xs text-muted-foreground">{{ t('payment.planCard.weeklyLimit') }}</span>
                    <div class="text-xl font-semibold text-foreground">${{ selectedPlan.weekly_limit_usd }}</div>
                  </div>
                  <div v-if="selectedPlan.monthly_limit_usd != null">
                    <span class="text-xs text-muted-foreground">{{ t('payment.planCard.monthlyLimit') }}</span>
                    <div class="text-xl font-semibold text-foreground">${{ selectedPlan.monthly_limit_usd }}</div>
                  </div>
                  <div v-if="selectedPlan.daily_limit_usd == null && selectedPlan.weekly_limit_usd == null && selectedPlan.monthly_limit_usd == null">
                    <span class="text-xs text-muted-foreground">{{ t('payment.planCard.quota') }}</span>
                    <div class="text-xl font-semibold text-foreground">{{ t('payment.planCard.unlimited') }}</div>
                  </div>
                </div>
              </div>
              <div v-if="enabledMethods.length >= 1" class="rounded-lg border border-border bg-card p-6">
              ...
              <div v-if="feeRate > 0 && selectedPlan.price > 0" class="rounded-lg border border-border bg-card p-6">
                <div class="space-y-2 text-sm">
                  <div class="flex justify-between">
                    <span class="text-muted-foreground">{{ t('payment.amountLabel') }}</span>
                    <span class="text-foreground">{{ formatSelectedPaymentAmount(subPaymentAmount) }}</span>
                  </div>
                  <div class="flex justify-between">
                    <span class="text-muted-foreground">{{ t('payment.fee') }} ({{ feeRate }}%)</span>
                  ...
                  <div class="flex justify-between border-t border-border pt-2">
                    <span class="font-medium text-foreground">{{ t('payment.actualPay') }}</span>
                    <span class="text-xl font-semibold text-foreground">{{ formatSelectedPaymentAmount(subTotalAmount) }}</span>
                  </div>
                </div>
              </div>
            ...
            <!-- Plan list -->
            <template v-else>
              <div v-if="checkout.plans.length === 0" class="rounded-lg border border-border bg-card py-16 text-center">
        ...
        <div v-if="(checkout.help_text || checkout.help_image_url) && paymentPhase === 'select' && !selectedPlan" class="rounded-lg border border-border bg-card p-4">
    ...
    <Teleport to="body">
      <Transition name="modal">
        <div v-if="previewImage" class="fixed inset-0 z-[60] flex items-center justify-center bg-black/70 backdrop-blur-sm" @click="previewImage = ''">
          <img :src="previewImage" alt="" class="max-h-[85vh] max-w-[90vw] rounded-lg object-contain shadow-xl" />
        </div>
      </Transition>
    </Teleport>
  </AppLayout>
...
// Subscription confirm: platform accent badge (clean card, no gradient)
