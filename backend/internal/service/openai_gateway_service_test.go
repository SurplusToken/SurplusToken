...
func TestOpenAISelectBestAccount_SharingRateStaysWithinCompactTier(t *testing.T) {
	...
	selected, compactBlocked, _ := svc.selectBestAccount(
		openAISharingRateSchedulingTestContext(true), nil, PlatformOpenAI, repo.accounts, "gpt-4", nil, true, "", true,
	)
...
func TestOpenAIBuildResponsesWebSocketURLIgnoresOAuthBaseURL(t *testing.T) {
	svc := &OpenAIGatewayService{cfg: &config.Config{
		Security: config.SecurityConfig{
			URLAllowlist: config.URLAllowlistConfig{
				Enabled:           false,
