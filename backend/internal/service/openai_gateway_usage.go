...
// contributionRewardRatePercent resolves the contribution reward rate for
// contributorUserID: a per-user override when set, else the platform default.
...
// contributionRewardFreezeHours resolves the platform-configured contribution
// reward freeze period (hours before a reward becomes spendable).
	...
	if responseModel := responseModelBillingDeclaration(
		...
		if identified, responseChannelPriced := s.hasIdentifiedOpenAIResponsePricing(ctx, responseModel, apiKey); identified {
			...
			responseCost, responseErr := s.calculateOpenAIRecordUsageCost(
				...
				videoMultiplier, baseMultiplier, tokens, serviceTier, longContextBillingGate, pricingAt, account.ID,
			)
