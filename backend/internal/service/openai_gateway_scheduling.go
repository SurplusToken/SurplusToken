...
func openAICompatibleAccountEligibilityFailureReasonBeforeProfit(ctx context.Context, account *Account, platform string, requestedModel string, requireCompact bool, requiredCapability OpenAIEndpointCapability) string {
	...
	if !account.IsSurplusAISchedulableType() {
		return "account_type_ineligible"
	}
	if stickyAccountSharingIneligible(ctx, account) {
		return "sharing_ineligible"
	}
	...
	if account.IsOpenAI() && !account.IsSurplusAIOwner(RequestingUserIDFromContext(ctx)) {
		if !account.IsSurplusAIContributionQuotaSchedulable() {
			return "contribution_protection"
		}
