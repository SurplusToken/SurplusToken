...
func (r *ModelPricingResolver) Resolve(ctx context.Context, input PricingInput) *ResolvedPricing {
	...
	if input.GroupID != nil && input.AccountID != nil && r.channelService != nil {
		if accountPricing := r.channelService.GetAccountModelPricing(ctx, *input.GroupID, *input.AccountID, input.Model); accountPricing != nil {
			resolved := r.resolveConfiguredPricing(accountPricing, input.Model, PricingSourceChannel)
...
// HasUsablePricing reports whether the resolved chain has a billable price.
...
func channelPricingUsable(pricing *ChannelModelPricing) bool {
	if pricing == nil {
		return false
	}
	...
	switch mode {
	case BillingModePerRequest, BillingModeImage, BillingModeVideo:
		if validPricingNumberPointer(pricing.PerRequestPrice) {
			return true
		}
		for i := range pricing.Intervals {
			if validPricingNumberPointer(pricing.Intervals[i].PerRequestPrice) {
				return true
			}
		}
		return false
	default:
		return validPricingNumberPointer(pricing.InputPrice) && validPricingNumberPointer(pricing.OutputPrice)
	}
}

func liteLLMTokenPricingUsable(pricing *LiteLLMModelPricing) bool {
	return pricing != nil && !pricing.TokenPricingAbsent &&
		validPricingNumber(pricing.InputCostPerToken) && validPricingNumber(pricing.OutputCostPerToken)
}

func liteLLMImagePricingUsable(pricing *LiteLLMModelPricing) bool {
	return pricing != nil && pricing.Mode == "image_generation" && validPositivePricingNumber(pricing.OutputCostPerImage)
}

func validPricingNumberPointer(value *float64) bool {
	return value != nil && validPricingNumber(*value)
}
...
func validPositivePricingNumber(value float64) bool {
	return validPricingNumber(value) && value > 0
}
...
func (r *ModelPricingResolver) resolveBasePricing(model string) (*ModelPricing, string) {
	...
	if err != nil {
		slog.Debug("failed to get model pricing from LiteLLM and built-in fallback",
...
func (r *ModelPricingResolver) applyTokenOverrides(chPricing *ChannelModelPricing, resolved *ResolvedPricing) {
	...
	if len(validIntervals) > 0 {
		...
		// 区间未命中时应回退到账号/分组/渠道配置的平价，而不是回退到
		// LiteLLM 或一个全零的 ModelPricing。
		applyFlatTokenOverrides(chPricing, resolved.BasePricing)
...
func applyFlatTokenOverrides(chPricing *ChannelModelPricing, pricing *ModelPricing) {
