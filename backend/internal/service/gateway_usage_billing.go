...
// 返回非 nil 的 ResolvedPricing 表示有可用定价，nil 表示定价不可用。
...
func (s *GatewayService) calculateTokenCost(
	...
	if s.resolver != nil && apiKey.Group != nil && opts.AccountID > 0 {
		gid := apiKey.Group.ID
		accountID := opts.AccountID
		resolved := s.resolver.Resolve(ctx, PricingInput{
			Model: billingModel, GroupID: &gid, Group: apiKey.Group, AccountID: &accountID,
		})
