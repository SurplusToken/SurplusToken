...
func TestCalculateSearchCost(t *testing.T) {
	t.Parallel()
	s := &BillingService{}
	require.Equal(t, 0.0, s.CalculateSearchCost(0, billingSearchAudioFloatPtr(10), 1).ActualCost)
	// nil price -> official xAI default $5/1k: 5 calls = 0.025
	require.InDelta(t, 0.025, s.CalculateSearchCost(5, nil, 1).ActualCost, 1e-9)
	// explicit 0 → free
	require.Equal(t, 0.0, s.CalculateSearchCost(5, billingSearchAudioFloatPtr(0), 1).ActualCost)
	price := 10.0
...
func billingSearchAudioFloatPtr(v float64) *float64 { return &v }
