...
// OpenAIAccountEligibility is a request-scoped hard scheduling constraint.
// A nil constraint preserves the legacy behavior and allows every otherwise
// compatible account.
type OpenAIAccountEligibility func(context.Context, *Account) bool
...
type OpenAIAccountScheduleRequest struct {
	...
	AccountEligible OpenAIAccountEligibility
}
...
func (s *defaultOpenAIAccountScheduler) selectBySessionHash(
	...
	if account == nil || !openAIStickyAccountMatchesGroup(account, req.GroupID) || !s.isAccountRequestCompatible(ctx, account, req) || !s.isAccountTransportCompatible(account, req.RequiredTransport) {
		clearBinding()
		return nil, false, nil
	}
...
func (s *defaultOpenAIAccountScheduler) isAccountRequestCompatibleReason(ctx context.Context, account *Account, req OpenAIAccountScheduleRequest) (bool, string) {
	...
	if stickyAccountSharingIneligible(ctx, account) {
		return false, "sticky_sharing_ineligible"
	}
	...
	if paused, decision := shouldAutoPauseOpenAIAccountByQuota(ctx, account); paused && !account.IsSurplusAIOwner(RequestingUserIDFromContext(ctx)) {
		if decision.reason != "" {
...
func (s *OpenAIGatewayService) SelectAccountWithSchedulerForCapability(
	...
	return s.selectAccountWithScheduler(ctx, groupID, previousResponseID, sessionHash, requestedModel, excludedIDs, requiredTransport, requiredCapability, "", requireCompact, platform, previousResponseCanMove, useUpstreamTokenCost, nil)
}

// SelectAccountWithSchedulerForCapabilityForRequest applies account-level
// client restrictions before an account is selected. Forward still repeats the
// same restriction check as a final defense against stale snapshots and callers
// that bypass scheduling.
func (s *OpenAIGatewayService) SelectAccountWithSchedulerForCapabilityForRequest(
	ctx context.Context,
	c *gin.Context,
	body []byte,
	groupID *int64,
	...
	eligibility := s.buildCodexClientAccountEligibility(c, body)
	return s.selectAccountWithScheduler(ctx, groupID, previousResponseID, sessionHash, requestedModel, excludedIDs, requiredTransport, requiredCapability, "", requireCompact, platform, previousResponseCanMove, useUpstreamTokenCost, eligibility)
}
...
func (s *OpenAIGatewayService) SelectAccountWithSchedulerForImages(
	...
	selection, decision, err := s.selectAccountWithScheduler(ctx, groupID, "", sessionHash, requestedModel, excludedIDs, OpenAIUpstreamTransportHTTPSSE, "", requiredCapability, false, PlatformOpenAI, false, false, nil)
	if err == nil && selection != nil && selection.Account != nil {
		return selection, decision, nil
	}
	// 如果要求 native 能力（如指定了模型）但没有可用的 APIKey 账号，回退到 basic（OAuth 账号）
	if requiredCapability == OpenAIImagesCapabilityNative {
		return s.selectAccountWithScheduler(ctx, groupID, "", sessionHash, requestedModel, excludedIDs, OpenAIUpstreamTransportHTTPSSE, "", OpenAIImagesCapabilityBasic, false, PlatformOpenAI, false, false, nil)
	}
	return selection, decision, err
}

func (s *OpenAIGatewayService) SelectAccountWithSchedulerForImagesForRequest(
	ctx context.Context,
	c *gin.Context,
	body []byte,
	groupID *int64,
	...
	eligibility := s.buildCodexClientAccountEligibility(c, body)
	selection, decision, err := s.selectAccountWithScheduler(ctx, groupID, "", sessionHash, requestedModel, excludedIDs, OpenAIUpstreamTransportHTTPSSE, "", requiredCapability, false, PlatformOpenAI, false, false, eligibility)
	if err == nil && selection != nil && selection.Account != nil {
		return selection, decision, nil
	}
	if requiredCapability == OpenAIImagesCapabilityNative {
		return s.selectAccountWithScheduler(ctx, groupID, "", sessionHash, requestedModel, excludedIDs, OpenAIUpstreamTransportHTTPSSE, "", OpenAIImagesCapabilityBasic, false, PlatformOpenAI, false, false, eligibility)
	}
	return selection, decision, err
}
...
func (s *OpenAIGatewayService) selectAccountWithScheduler(
	...
	selection, decision, err := s.selectAccountWithSchedulerOnce(ctx, groupID, previousResponseID, sessionHash, requestedModel, excludedIDs, requiredTransport, requiredCapability, requiredImageCapability, requireCompact, platform, previousResponseCanMove, useUpstreamTokenCost, accountEligibility)
	...
	return s.selectAccountWithSchedulerOnce(withOpenAIProxyStreamQuarantineBypass(ctx), groupID, previousResponseID, sessionHash, requestedModel, excludedIDs, requiredTransport, requiredCapability, requiredImageCapability, requireCompact, platform, previousResponseCanMove, useUpstreamTokenCost, accountEligibility)
}
...
func (s *OpenAIGatewayService) selectAccountWithSchedulerOnce(
	...
	if scheduler == nil {
		...
		if requiredTransport == OpenAIUpstreamTransportAny || requiredTransport == OpenAIUpstreamTransportHTTPSSE {
			...
			for {
				selection, err := s.selectAccountWithLoadAwareness(ctx, groupID, platform, legacySessionHash, requestedModel, effectiveExcludedIDs, requireCompact, requiredCapability, useUpstreamTokenCost, accountEligibility)
				if err != nil {
					return nil, decision, err
				}
				if selection == nil || selection.Account == nil {
	...
	return scheduler.Select(ctx, OpenAIAccountScheduleRequest{
		...
		RequireCompact:          requireCompact,
		ExcludedIDs:             excludedIDs,
		AccountEligible:         accountEligibility,
	})
}

func matchesOpenAIAccountEligibility(ctx context.Context, account *Account, eligibility OpenAIAccountEligibility) bool {
	return eligibility == nil || eligibility(ctx, account)
}
