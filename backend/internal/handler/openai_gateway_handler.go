...
func openAICompatibleRequestPlatform(ctx context.Context, apiKey *service.APIKey) string {
	if platform, ok := service.ResolvedTargetPlatformFromContext(ctx); ok {
		switch platform {
		case service.PlatformGrok, service.PlatformKimi, service.PlatformZhipu:
			return platform
		}
...
func allowOpenAICompatibleMessagesDispatch(apiKey *service.APIKey) bool {
	...
	if apiKey.Group.Platform == service.PlatformKimi {
		return true
	}
	if apiKey.Group.Platform == service.PlatformZhipu {
		return true
	}
...
func openAICompatibleTextTargetAllowed(c *gin.Context, apiKey *service.APIKey, model string) bool {
	return compositeTargetPlatformAllowed(c, apiKey, model, service.PlatformOpenAI, service.PlatformGrok, service.PlatformKimi, service.PlatformZhipu)
}
...
func (h *OpenAIGatewayHandler) Responses(c *gin.Context) {
	...
	for {
		...
		selection, scheduleDecision, err := h.gatewayService.SelectAccountWithSchedulerForCapabilityForRequest(
			c.Request.Context(),
			c,
			body,
			apiKey.GroupID,
