...
func (h *GatewayHandler) compositeAvailableModels(ctx context.Context, groupID *int64) []string {
	...
	for _, platform := range []string{service.PlatformAnthropic, service.PlatformGemini, service.PlatformOpenAI, service.PlatformAntigravity, service.PlatformGrok, service.PlatformKimi, service.PlatformZhipu} {
...
func defaultModelIDsForPlatform(platform string) []string {
	...
	case service.PlatformComposite:
		ids := make([]string, 0)
		seen := make(map[string]struct{})
		for _, concretePlatform := range []string{service.PlatformAnthropic, service.PlatformGemini, service.PlatformOpenAI, service.PlatformAntigravity, service.PlatformGrok, service.PlatformKimi, service.PlatformZhipu} {
...
func (h *GatewayHandler) calculateSubscriptionRemaining(group *service.Group, sub *service.UserSubscription) float64 {
	...
	// 检查周限额（订阅级覆盖优先）
	if weeklyLimit := sub.EffectiveWeeklyLimit(group); weeklyLimit != nil && *weeklyLimit > 0 {
		remaining := *weeklyLimit - sub.WeeklyUsageUSD
		if remaining <= 0 {
			return 0
		}
		remainingValues = append(remainingValues, remaining)
