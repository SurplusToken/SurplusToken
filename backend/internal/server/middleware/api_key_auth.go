...
func apiKeyAuthWithSubscription(apiKeyService *service.APIKeyService, subscriptionService *service.SubscriptionService, cfg *config.Config, settingService *service.SettingService) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		// These read-only endpoints must remain available after the request that
		// generated their data consumes the key's remaining balance.
			...
			if subscription != nil {
				...
				if validateErr != nil {
					...
					if errors.Is(validateErr, service.ErrDailyLimitExceeded) ||
						...
						errors.Is(validateErr, service.ErrCarpoolSharedPoolExhausted) {
