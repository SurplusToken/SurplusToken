...
func (r *remoteSessionRepository) Create(ctx context.Context, s *service.RemoteSession) error {
	const q = `
		INSERT INTO remote_sessions
			(account_id, surplus_user_id, kasm_id, kasm_user_id, mode, status, connect_url, created_at, last_seen_at)
		VALUES ($1, $2, $3, $4, $5, $6, $7, NOW(), NOW())
		RETURNING id, created_at, last_seen_at
	`
	row := r.sql.QueryRowContext(ctx, q,
		s.AccountID, s.SurplusUserID, s.KasmID, s.KasmUserID, s.Mode, s.Status, s.ConnectURL)
...
func (r *remoteSessionRepository) ListLive(ctx context.Context) ([]service.RemoteSession, error) {
	const q = `
		SELECT id, account_id, surplus_user_id, kasm_id, kasm_user_id, mode, status, connect_url,
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			&s.ID, &s.AccountID, &s.SurplusUserID, &s.KasmID, &s.KasmUserID, &s.Mode, &s.Status, &s.ConnectURL,
...
// or (nil, nil) when none exists. Used to reattach a reconnecting user to their container.
func (r *remoteSessionRepository) GetLiveByAccountAndUser(ctx context.Context, accountID, surplusUserID int64) (*service.RemoteSession, error) {
	const q = `
		SELECT id, account_id, surplus_user_id, kasm_id, kasm_user_id, mode, status, connect_url,
		       created_at, last_seen_at, ended_at
		FROM remote_sessions
		WHERE account_id = $1
		  AND surplus_user_id = $2
		  AND status IN ('starting', 'running')
		  AND ended_at IS NULL
		ORDER BY id DESC
		LIMIT 1
	...
	var s service.RemoteSession
	var endedAt sql.NullTime
	err := r.sql.QueryRowContext(ctx, q, accountID, surplusUserID).Scan(
		&s.ID, &s.AccountID, &s.SurplusUserID, &s.KasmID, &s.KasmUserID, &s.Mode, &s.Status, &s.ConnectURL,
		&s.CreatedAt, &s.LastSeenAt, &endedAt,
	)
