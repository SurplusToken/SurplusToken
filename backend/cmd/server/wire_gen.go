...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	carpoolCommonsCounter := repository.NewCarpoolCommonsCache(redisClient, db)
	carpoolUpstreamWindowSource := repository.NewCarpoolUpstreamWindowRepository(db)
	billingCacheService := service.ProvideBillingCacheService(billingCache, userRepository, userSubscriptionRepository, apiKeyRepository, userRPMCache, userGroupRateRepository, configConfig, serviceUserPlatformQuotaRepository, carpoolCommonsCounter, carpoolUpstreamWindowSource)
	apiKeyCache := repository.NewAPIKeyCache(redisClient)
	concurrencyCache := repository.ProvideConcurrencyCache(redisClient, configConfig)
	schedulerCache := repository.ProvideSchedulerCache(redisClient, configConfig)
	accountRepository := repository.NewAccountRepository(client, db, schedulerCache, redisClient)
	...
	carpoolBillingCycleRecorder := repository.NewCarpoolBillingCycleRepository(db)
	subscriptionService := service.ProvideSubscriptionService(groupRepository, userSubscriptionRepository, billingCacheService, client, configConfig, carpoolUpstreamWindowSource, carpoolBillingCycleRecorder)
	...
	openAIGatewayService := service.NewOpenAIGatewayService(accountRepository, usageLogRepository, usageBillingRepository, userRepository, userSubscriptionRepository, userGroupRateRepository, gatewayCache, configConfig, schedulerSnapshotService, concurrencyService, billingService, rateLimitService, billingCacheService, httpUpstream, deferredService, openAITokenProvider, grokTokenProvider, modelPricingResolver, channelService, balanceNotifyService, settingService, serviceUserPlatformQuotaRepository, gatewayService)
	...
	chatDB := repository.ProvideChatDB(configConfig)
	chatRepository := repository.NewChatRepository(chatDB)
	chatService := service.ProvideChatService(chatRepository, configConfig)
	chatHandler := handler.NewChatHandler(chatService, apiKeyService)
	...
	accountService := service.ProvideAccountService(accountRepository, groupRepository, proxyRepository, settingService, subscriptionService, apiKeyService)
	...
	proxyService := service.ProvideProxyService(proxyRepository, proxyExitInfoProber)
	...
	remoteSessionRepository := repository.NewRemoteSessionRepository(db)
	kasmClient := service.ProvideKasmClient(configConfig)
	...
	accountHandler := admin.ProvideAccountHandler(adminService, oAuthService, openAIOAuthService, kimiOAuthService, geminiOAuthService, antigravityOAuthService, grokOAuthService, rateLimitService, accountUsageService, accountTestService, concurrencyService, crsSyncService, sessionLimitCache, rpmCache, compositeTokenCacheInvalidator, grokQuotaService)
	...
	contributionWithdrawalHandler := admin.NewContributionWithdrawalHandler(contributionService)
	ollamaCloudUsageService := service.ProvideOllamaCloudUsageService(accountRepository, httpUpstream, settingService, secretEncryptor, configConfig, leaderLockCache, db)
	adminHandlers := handler.ProvideAdminHandlers(dashboardHandler, adminUserHandler, groupHandler, accountHandler, adminAnnouncementHandler, dataManagementHandler, backupHandler, oAuthHandler, openAIOAuthHandler, geminiOAuthHandler, antigravityOAuthHandler, grokOAuthHandler, kimiOAuthHandler, proxyHandler, adminRedeemHandler, promoHandler, settingHandler, opsHandler, systemHandler, adminSubscriptionHandler, adminUsageHandler, userAttributeHandler, errorPassthroughHandler, tlsFingerprintProfileHandler, adminAPIKeyHandler, scheduledTestHandler, channelHandler, channelMonitorHandler, channelMonitorRequestTemplateHandler, contentModerationHandler, promptAdminHandler, paymentHandler, affiliateHandler, complianceHandler, auditLogHandler, upstreamBillingProbeService, contributionWithdrawalHandler, ollamaCloudUsageService)
	...
	availableChannelHandler := handler.NewAvailableChannelHandler(channelService, gatewayService, apiKeyService, settingService, opsService)
	...
	carpoolSettlementScheduler := service.ProvideCarpoolSettlementScheduler(carpoolService, leaderLockCache, db)
