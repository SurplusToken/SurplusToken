...
type Group struct {
	...
	// 是否为动态共享池；启用后仅调度用户贡献账号并应用账号共享报价
	DynamicSharingPool bool `json:"dynamic_sharing_pool,omitempty"`
	// 是否启用高峰时段倍率
	...
	// 是否为贡献者自用专属分组（系统自动维护，对外隐藏）
	AutoSelfUse bool `json:"auto_self_use,omitempty"`
	// 是否允许该分组使用图片生成能力
	...
	for i := range columns {
		...
		case group.FieldDynamicSharingPool:
			if value, ok := values[i].(*sql.NullBool); !ok {
				return fmt.Errorf("unexpected type %T for field dynamic_sharing_pool", values[i])
			} else if value.Valid {
				_m.DynamicSharingPool = value.Bool
			}
		...
		case group.FieldAutoSelfUse:
			if value, ok := values[i].(*sql.NullBool); !ok {
				return fmt.Errorf("unexpected type %T for field auto_self_use", values[i])
			} else if value.Valid {
				_m.AutoSelfUse = value.Bool
			}
...
func (_m *Group) String() string {
	...
	builder.WriteString(", ")
	builder.WriteString("dynamic_sharing_pool=")
	builder.WriteString(fmt.Sprintf("%v", _m.DynamicSharingPool))
	builder.WriteString(", ")
	...
	builder.WriteString(", ")
	builder.WriteString("auto_self_use=")
	builder.WriteString(fmt.Sprintf("%v", _m.AutoSelfUse))
	builder.WriteString(", ")
